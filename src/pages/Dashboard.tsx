...
import WritePostTool from '../components/tabs/manual/WritePostTool';
import ReviewReplyTool from '../components/tabs/manual/ReviewReplyTool';
import QAAnswerTool from '../components/tabs/manual/QAAnswerTool';
import ImageProcessorTool from '../components/tabs/manual/ImageProcessorTool';
import ContentLibrary from '../components/tabs/manual/ContentLibrary';
...
// First 5 are Manual AI Tools; the rest are automated/profile tabs
const MANUAL_TOOL_IDS: TabId[] = ['write-post', 'reply-review', 'answer-qa', 'process-images', 'content-library'];

const TOOL_HEADINGS: Partial<Record<TabId, { title: string; subtitle: string }>> = {
  'write-post':     { title: 'Write a GBP Post', subtitle: 'Upload a photo, answer 3 quick questions, and Claude writes a polished post you can copy into GBP.' },
  'reply-review':   { title: 'Reply to a Review', subtitle: "Paste the review, select the star rating — Claude mirrors the customer's tone and adds local SEO naturally." },
  'answer-qa':      { title: 'Answer a Customer Question', subtitle: 'Paste the Q&A question and choose a style. SEO Mode weaves in your city and service keywords.' },
  'process-images': { title: 'Process Images for GBP', subtitle: 'Upload up to 10 photos. Claude Vision captions each one. EXIF, GPS, and SEO filenames are injected automatically.' },
  'content-library':{ title: 'Content Library', subtitle: 'Every post, reply, and answer Claude has written for you. Mark as posted to update your AI Memory.' },
};

const TAB_ICONS: Record<TabId, React.ReactNode> = {
  'write-post':     <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>,
  'reply-review':   <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>,
  'answer-qa':      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>,
  'process-images': <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>,
  'content-library':<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>,
...
const AI_TABS: TabId[] = ['write-post', 'reply-review', 'answer-qa', 'process-images', 'content-library'];

const SECTIONS_BY_CATEGORY: Record<string, TabId[]> = {
  contractor:  [...AI_TABS, 'profile', 'reviews', 'qa', 'photos', 'posts', 'insights', 'services', 'products', 'bookings', 'getreviews', 'memory'],
  restaurant:  [...AI_TABS, 'profile', 'reviews', 'qa', 'photos', 'posts', 'insights', 'products', 'bookings', 'getreviews', 'memory'],
  store:       [...AI_TABS, 'profile', 'reviews', 'qa', 'photos', 'posts', 'insights', 'products', 'getreviews', 'memory'],
  salon:       [...AI_TABS, 'profile', 'reviews', 'qa', 'photos', 'posts', 'insights', 'services', 'bookings', 'getreviews', 'memory'],
  hotel:       [...AI_TABS, 'profile', 'reviews', 'qa', 'photos', 'posts', 'insights', 'services', 'bookings', 'getreviews', 'memory'],
  doctor:      [...AI_TABS, 'profile', 'reviews', 'qa', 'photos', 'posts', 'insights', 'services', 'bookings', 'getreviews', 'memory'],
  real_estate: [...AI_TABS, 'profile', 'reviews', 'qa', 'photos', 'posts', 'insights', 'services', 'getreviews', 'memory'],
};
...
export default function Dashboard() {
  ...
  const [activeTab, setActiveTab] = useState<TabId>('write-post');
  ...
  const freeTabIds: TabId[] = [...AI_TABS, 'profile', 'posts', 'products', 'bookings', 'getreviews', 'memory', 'manual'];
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* AI Tools group label */}
          <span style={{ fontSize: '0.6rem', fontWeight: 700, letterSpacing: '0.1em', color: '#4f8ef7', textTransform: 'uppercase', padding: '0 0.5rem 0 0.25rem', whiteSpace: 'nowrap', opacity: 0.7 }}>
            AI Tools
          </span>
          {(isGuest ? SECTIONS_BY_CATEGORY[demoCategory] ?? TABS.map(tab => tab.id) : TABS.map(tab => tab.id).filter(id => id !== 'owner' || client?.isOwner).filter(id => id !== 'manual')).flatMap((tabId, idx) => {
            const isActive = activeTab === tabId;
            const isFirstAuto = tabId === 'profile' && idx > 0;
            const btn = (
              <button
                key={tabId}
            ...
            if (isFirstAuto) {
              return [
                <div key="sep" style={{ width: 1, height: 20, background: 'rgba(255,255,255,0.12)', margin: '0 0.375rem', alignSelf: 'center', flexShrink: 0 }} />,
                <span key="auto-label" style={{ fontSize: '0.6rem', fontWeight: 700, letterSpacing: '0.1em', color: 'rgba(232,238,255,0.25)', textTransform: 'uppercase', padding: '0 0.375rem 0 0', whiteSpace: 'nowrap' }}>
                  Profile & Auto
                </span>,
                btn,
              ];
      ...
      >
        <div className="max-w-3xl mx-auto">
          ...
          {/* ── AI Tools (Manual Mode) ─── */}
          {MANUAL_TOOL_IDS.includes(activeTab) && (() => {
            const h = TOOL_HEADINGS[activeTab];
            return (
              <>
                {h && (
                  <div style={{ marginBottom: '1.5rem' }}>
                    <h2 style={{ fontSize: '1.125rem', fontWeight: 700, color: 'rgba(232,238,255,0.95)', marginBottom: '0.375rem' }}>{h.title}</h2>
                    <p style={{ fontSize: '0.8125rem', color: 'rgba(232,238,255,0.45)', lineHeight: 1.5 }}>{h.subtitle}</p>
                  </div>
                )}
                {activeTab === 'write-post'     && <WritePostTool isGuest={isGuest} />}
                {activeTab === 'reply-review'   && <ReviewReplyTool isGuest={isGuest} />}
                {activeTab === 'answer-qa'      && <QAAnswerTool isGuest={isGuest} />}
                {activeTab === 'process-images' && <ImageProcessorTool isGuest={isGuest} />}
                {activeTab === 'content-library'&& <ContentLibrary isGuest={isGuest} />}
              </>
            );
          })()}
